...
def continuous_dubins_car_leaky(state, action, env):
...
# Leaky reward function for Continuous Dubins Car
def continuous_dubins_car_w_velocity_leaky(state, action, env):
    goal_x, goal_y, goal_boundary = env.goal_x, env.goal_y, env.goal_boundary
    x, y = state[0] , state[1]
    distance_from_goal = euclidean_distance((x, y), (goal_x, goal_y))
...
def mountain_car_leaky(state, action, env):
    beta = 1
...
#
#
def continuous_mountain_car_leaky(state, action, env):
    beta = 1
    position, velocity = state[0], state[1]
    position_greater_than_goal = greater_than_or_eq_to(position, env.goal_position, sparsity_multiplier=beta)
    velocity_grater_than_goal = greater_than_or_eq_to(velocity, env.goal_velocity, sparsity_multiplier=beta)
