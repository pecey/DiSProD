...
class ContinuousDubinsCar(gym.Env):
    ...
    def __init__(self, alpha=0.0, add_obstacle=False, config_file=None, map_name="random"):
        ...
        self.min_y_position = -25
        self.min_x_position = -25
        self.max_y_position = 25
        self.max_x_position = 25
        self.alpha = alpha
        ...
        self.low_state = np.array([self.min_x_position, self.min_y_position, self.min_theta , self.min_velocity , self.min_angular_velocity], dtype=np.float32)
        self.high_state = np.array([self.max_x_position, self.max_y_position, self.max_theta , self.max_velocity , self.max_angular_velocity], dtype=np.float32)
    ...
    def step(self, action):
        reward = 0
        linear_velocity, angular_velocity = action.squeeze()
        ...
        self.x, self.y, self.theta = self.update_state(self.x, self.y, self.theta, angular_velocity * self.angular_velocity_multiplier, linear_velocity, self.time_interval, noise)

        # Check if the car is within the boundary around the goal
        # large_boundary = 2
        # if np.isclose(self.x, self.goal_x, atol=large_boundary) and np.isclose(self.y, self.goal_y, atol=large_boundary):
        #     #self.done = True
        #     # Switch goals if nearby
        #     if self.goal_x == 0:
        #         self.goal_x = -15.8
        #         self.goal_y = 8.4
        #     else:
        ...
        observation = self.x, self.y, self.theta, linear_velocity , angular_velocity
    ...
    def reset(self):
        ...
        if self.config_file is not None:
            ...
            selected_config = np.random.choice(configs) if self.config_num == -1 else configs[self.config_num]
        ...
        self.done = False
        return np.array((self.x, self.y, self.theta , 0 , 0))
    ...
    def reset_canvas(self):
        ...
        inner_dim_a = 10.60 * 2
        inner_dim_b = 4.60 * 2
        alpha = (1.57 + 0.78)/DEGREE_TO_RADIAN_MULTIPLIER
        self.ax.add_patch(Ellipse((xcenter, ycenter), outer_dim_a, outer_dim_b, angle=alpha, linewidth=2, fill=False, zorder=2))
        self.ax.add_patch(Ellipse((xcenter, ycenter), inner_dim_a, inner_dim_b, angle=alpha, linewidth=2, fill=False, zorder=2))
        ...
        if self.plot_boundary:
            for boundary in self.boundary:
                self.ax.add_patch(Rectangle((boundary["x"], boundary["y"]), boundary["width"], boundary["height"], color='gray'))

    def set_imagined_trajectory(self, path):
