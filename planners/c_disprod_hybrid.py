...
class ContinuousDisprod(Disprod):
    def __init__(self, env, cfg, key):
        ...
        if cfg["disprod"]["taylor_expansion_mode"] == "complete":
            ...
            self.dynamics_dist_fn = dynamics_comp_hybrid(self.ns_fn, env, fop_fn, sop_fn, self.noise_dist, self.n_real_var, self.n_bin_var)
...
def dynamics_comp_hybrid(ns_fn, env, fop_fn, sop_fn, noise_dist, n_real_var, n_bin_var):
    def _dynamics_comp_hybrid(s_mu, s_var, a_mu, a_var, key):
        ns = ns_fn(s_mu, a_mu, env)
        ...
        mu_for_bin = jnp.clip(next_state_expectation[n_real_var:n_real_var + n_bin_var], 0, 1)
        next_state_expectation = next_state_expectation.at[n_real_var:n_real_var + n_bin_var].set(mu_for_bin)
        
        var_for_bin = mu_for_bin*(1-mu_for_bin)
        next_state_variance = next_state_variance.at[n_real_var:n_real_var + n_bin_var].set(var_for_bin)
