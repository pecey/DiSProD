...
# CEM
jid1=$(sbatch --export=ALL,base_run_name=${base_run_name},alg=cem,noisy=${noisy} --parsable -o ${SLURM_LOG_PATH}/${identifier}_cem.out  -e ${SLURM_LOG_PATH}/${identifier}_cem.err ${DISPROD_PATH}/slurm_scripts/${system}/${exp_name}/${env_name}.sh)

# MPPI
jid2=$(sbatch --export=ALL,base_run_name=${base_run_name},alg=mppi,noisy=${noisy} --parsable -o ${SLURM_LOG_PATH}/${identifier}_mppi.out  -e ${SLURM_LOG_PATH}/${identifier}_mppi.err ${DISPROD_PATH}/slurm_scripts/${system}/${exp_name}/${env_name}.sh)

# DiSProD
jid3=$(sbatch --export=ALL,base_run_name=${base_run_name},alg=disprod,noisy=${noisy} --parsable -o ${SLURM_LOG_PATH}/${identifier}_disprod.out  -e ${SLURM_LOG_PATH}/${identifier}_disprod.err ${DISPROD_PATH}/slurm_scripts/${system}/${exp_name}/${env_name}.sh)
