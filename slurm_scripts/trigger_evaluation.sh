...
base_run_name=${today}-evaluation
echo "Triggering evaluation with ${env_name} using ${alg} on ${system}"

SLURM_LOG_PATH=${HOME}/awesome-sogbofa/slurm_logs/${env_name}/evaluation/${today}
...
env_mapping['half_cheetah']="half-cheetah-v1"

# Run experiments
jid1=$(sbatch --export=ALL,alg=${alg},base_run_name=${base_run_name} --parsable -o ${SLURM_LOG_PATH}/${identifier}_main.out  -e ${SLURM_LOG_PATH}/${identifier}_main.err ${HOME}/awesome-sogbofa/slurm_scripts/${system}/evaluation/${env_name}.sh)
