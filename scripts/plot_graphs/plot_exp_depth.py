...
def main():
    ...
    plot_file_name =  f"exp_depth" if not noisy else f"exp_depth_noisy"
    ...
    for algorithm in algorithms:
        ...
        for depth in depths:
            ...
            path_to_rewards = f"{results_base_path}/{run_name}-{algorithm}-{depth}/logs/rewards.mat"
            ...
            statistics_grouped[algorithm][depth] = {"mean": np.mean(mean_groups), "sd": np.std(mean_groups)}
    ...
    for algorithm in algorithms:
        ...
        sd = []
        for depth in depths:
            mean.append(statistics_grouped[algorithm][depth]["mean"])
            sd.append(statistics_grouped[algorithm][depth]["sd"])
        mean = np.array(mean)
        sd = np.array(sd)
        plt.plot(x, mean, label=alg_mapping[algorithm])
        plt.fill_between(x, mean-sd, mean+sd, alpha=0.2)
